...
First of all, we need some data to work with. If the data is stored in a `cvs`,
...
PS: if you just need a single variable, you could use `pull`.
...
mean_air_time <- flights %>% pull(air_time) %>% mean(na.rm = TRUE)
flights %>% 
  select(origin, air_time) %>%
  filter(origin == "JFK", air_time > mean_air_time)

# what if there is a name colision?
air_time <- flights %>% pull(air_time) %>% mean(na.rm = TRUE)
flights %>% 
  select(origin, air_time) %>%
...
flight_distances <- flights %>% 
  transmute(distance, 
            distance_type = case_when(distance < 500 ~ "short", 
                                      distance < 1000 ~ "mid", 
                                      TRUE ~ "long" ))
flight_distances
```
...
```{r}
flights %>% group_by(tailnum) %>% summarize()
flights %>% group_by(tailnum) %>% tally() # shorthand
