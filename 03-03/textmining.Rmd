...
See here for a nice comparsion between the packages: https://quanteda.io/articles/pkgdown/comparison.html
...
news_df <- tibble(
  ...
  mutate(url = str_c("http://news.yahoo.com", url), id = sprintf("%02d", row_number())) %>% 
  head(20)
```
...
We try to study the bigrams in Derivative wiki page.
```{r}
...
news_tokens2 %>% 
  ...
  mutate(if_else(word1 %in% negate_words && sentiment == "negative", "positive", "negative")) %>% 
  mutate(if_else(word1 %in% negate_words && sentiment == "positive", "negative", "positive")) %>% 
  group_by(id) %>% 
  summarize(
    positive = sum(sentiment == "positive", na.rm = TRUE), 
    negative = sum(sentiment == "negative", na.rm = TRUE), 
...
```{r}
tibble(topic = cutree(h, k = 4), title = news_df$title) %>% 
  arrange(topic) %>% 
  mutate(title = str_trunc(title, 80)) %>% 
  select(topic, title)
```
...
Ok, there is only one dominate topic in most documents.

Let's use the most probable topic to classify a document.
```{r}
tidy(news_lda, matrix = "gamma") %>% 
  group_by(document) %>% 
  filter(row_number(gamma) == n()) %>% 
  left_join(news_df, by = c("document" = "id")) %>% 
  arrange(document) %>% 
  mutate(title = str_trunc(title, 80)) %>% 
  select(topic, title)
```
